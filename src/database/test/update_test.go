...
func TestUpdateDatabase_SameName(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodGet, "/api/databases/update/testdb", updateBody)
...
func TestUpdateDatabase_AlreadyExists(t *testing.T) {
	...
	rCreate1 := httptest.NewRequest(http.MethodGet, "/api/databases/create/", body1)
	wCreate1 := httptest.NewRecorder()
	database.HandleCreateDatabase(wCreate1, rCreate1, dataDir)

	body2 := bytes.NewBufferString(`{"name":"db2"}`)
	rCreate2 := httptest.NewRequest(http.MethodGet, "/api/databases/create/", body2)
	...
	r := httptest.NewRequest(http.MethodGet, "/api/databases/update/db1", updateBody)
	w := httptest.NewRecorder()
	resp := database.HandleUpdateDatabase(w, r, dataDir)
	if resp.Success || resp.Error == nil || resp.Error.Code != "ERR_DB_EXISTS" {
		t.Errorf("Update auf existierenden Namen nicht korrekt erkannt: %+v", resp)
	}
}
